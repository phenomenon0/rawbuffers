/*
 * vizvim.c — Vizvim creative capture WASM core
 *
 * Exports pixel buffer + effect functions for the JS frontend.
 * No rb_run loop — JS drives the frame loop.
 *
 * Build: emcc -O2 -o vizvim.js vizvim.c -s MODULARIZE=1 -s EXPORT_NAME='RawBuf' ...
 */
...
static float face_landmarks[12]; /* 6 landmarks x 2 coords */
...
int vz_init(int w, int h) {
    /* Set canvas dimensions (Vizvim uses preview-canvas) */
    EM_ASM({ var c = document.getElementById('preview-canvas'); if(c){ c.width = $0; } }, w);
    EM_ASM({ var c = document.getElementById('preview-canvas'); if(c){ c.height = $0; } }, h);
...
EMSCRIPTEN_KEEPALIVE
int vz_get_buffer(void) { return (int)surface.back; }

EMSCRIPTEN_KEEPALIVE
int vz_width(void) { return surface.width; }

EMSCRIPTEN_KEEPALIVE
int vz_height(void) { return surface.height; }

EMSCRIPTEN_KEEPALIVE
void vz_present(void) {
    rb_present(&surface);
}
...
static void effect_vj_tool(void) {
    /* JS chains multiple effects — this is a no-op, JS calls vz_apply_effect multiple times */
}
...
EMSCRIPTEN_KEEPALIVE
void vz_set_chroma(int kr, int kg, int kb, int threshold, int br, int bg, int bb) {
    chroma_key_r = kr;
    chroma_key_g = kg;
    chroma_key_b = kb;
...
EMSCRIPTEN_KEEPALIVE
void vz_set_channel_split(int mode, int offset) {
    channel_split_mode = mode;
    channel_split_offset = offset;
}

EMSCRIPTEN_KEEPALIVE
void vz_set_audio_data(float bass, float mid, float high, int beat) {
    audio_bass = bass;
    audio_mid = mid;
    audio_high = high;
    audio_beat = beat;
...
EMSCRIPTEN_KEEPALIVE
int vz_get_waveform_buffer(void) {
    return (int)audio_waveform;
}

EMSCRIPTEN_KEEPALIVE
int vz_get_mask_buffer(void) {
    int total = surface.width * surface.height;
...
EMSCRIPTEN_KEEPALIVE
void vz_set_ml_bg_color(int r, int g, int b) {
    ml_bg_color = RB_RGB(r, g, b);
}

EMSCRIPTEN_KEEPALIVE
void vz_set_det_box(int idx, int x, int y, int w, int h, const char *label) {
...
EMSCRIPTEN_KEEPALIVE
void vz_set_det_count(int n) {
    det_count = n > 20 ? 20 : n;
}

EMSCRIPTEN_KEEPALIVE
void vz_set_pose(int idx, float x, float y) {
    if (idx >= 0 && idx < 33) {
        pose_x[idx] = x;
        pose_y[idx] = y;
    }
...
EMSCRIPTEN_KEEPALIVE
void vz_set_pose_valid(int v) { pose_valid = v; }

EMSCRIPTEN_KEEPALIVE
void vz_set_face(int x, int y, int w, int h) {
    face_x = x; face_y = y; face_w = w; face_h = h;
}

EMSCRIPTEN_KEEPALIVE
void vz_set_face_landmarks(int idx, float x, float y) {
    if (idx >= 0 && idx < 6) {
...
EMSCRIPTEN_KEEPALIVE
void vz_set_face_valid(int v) { face_valid = v; }

EMSCRIPTEN_KEEPALIVE
int vz_get_depth_buffer(void) {
    int total = surface.width * surface.height;
...
EMSCRIPTEN_KEEPALIVE
int vz_get_pip_buffer(int w, int h) {
    int total = w * h;
...
EMSCRIPTEN_KEEPALIVE
int vz_get_annotation_buffer(void) {
    int total = surface.width * surface.height;
...
EMSCRIPTEN_KEEPALIVE
void vz_annotation_clear(void) {
    if (annotation_buffer && annotation_size > 0)
        memset(annotation_buffer, 0, annotation_size * sizeof(uint32_t));
}

EMSCRIPTEN_KEEPALIVE
void vz_annotation_line(int x0, int y0, int x1, int y1, int r, int g, int b) {
...
EMSCRIPTEN_KEEPALIVE
void vz_set_subtitle(const char *text) {
...
EMSCRIPTEN_KEEPALIVE
void vz_set_thermal(int palette) { thermal_palette = palette; }

EMSCRIPTEN_KEEPALIVE
void vz_set_edge(int thr, int mono) { edge_threshold = thr; edge_mono = mono; }

EMSCRIPTEN_KEEPALIVE
void vz_set_nightvision(float gain, int noise) { nv_gain = gain; nv_noise = noise; }

EMSCRIPTEN_KEEPALIVE
void vz_set_vhs(int aber, int noise_freq) { vhs_aberration = aber; vhs_noise_freq = noise_freq; }

EMSCRIPTEN_KEEPALIVE
void vz_set_glitch(int intensity, int block) { glitch_intensity = intensity; glitch_block_size = block; }

EMSCRIPTEN_KEEPALIVE
void vz_set_pixelate(int block) { pixel_block = block; }

EMSCRIPTEN_KEEPALIVE
void vz_set_old_film(int dust, int scratch, float flicker) { film_dust = dust; film_scratch_freq = scratch; film_flicker = flicker; }

EMSCRIPTEN_KEEPALIVE
void vz_set_fft_gains(float b, float m, float h) { fft_bass_gain = b; fft_mid_gain = m; fft_high_gain = h; }

EMSCRIPTEN_KEEPALIVE
void vz_set_waveform(int r, int g, int b, int pos) { wave_color_r = r; wave_color_g = g; wave_color_b = b; wave_position = pos; }

EMSCRIPTEN_KEEPALIVE
void vz_set_pose_colors(int br, int bg, int bb, int jr, int jg, int jb) { pose_bone_r=br; pose_bone_g=bg; pose_bone_b=bb; pose_joint_r=jr; pose_joint_g=jg; pose_joint_b=jb; }

EMSCRIPTEN_KEEPALIVE
void vz_set_face_params(int mode, int block) { face_mode = mode; face_block = block; }

EMSCRIPTEN_KEEPALIVE
void vz_set_fog(int r, int g, int b, float intensity) { fog_r=r; fog_g=g; fog_b=b; fog_intensity=intensity; }

EMSCRIPTEN_KEEPALIVE
void vz_set_annot_brush(int size) { annot_brush_size = size; }

EMSCRIPTEN_KEEPALIVE
void vz_set_subtitle_scale(int scale) { subtitle_font_scale = scale; }

/* ─── Adjustment setters ─── */

EMSCRIPTEN_KEEPALIVE
void vz_set_brightness(int val) { adj_brightness = val; }

EMSCRIPTEN_KEEPALIVE
void vz_set_contrast(int val) { adj_contrast = val; }

EMSCRIPTEN_KEEPALIVE
void vz_set_saturation(int val) { adj_saturation = val; }

EMSCRIPTEN_KEEPALIVE
void vz_set_hue_rotate(int deg) { adj_hue = deg; }

EMSCRIPTEN_KEEPALIVE
void vz_flip_h(void) { flip_h = !flip_h; }

EMSCRIPTEN_KEEPALIVE
void vz_flip_v(void) { flip_v = !flip_v; }

/* ─── Adjustment application ─── */

EMSCRIPTEN_KEEPALIVE
void vz_apply_adjustments(void) {
    if (!adj_brightness && !adj_contrast && !adj_saturation && !adj_hue && !flip_h && !flip_v) return;
    int w = surface.width, h = surface.height;
    uint32_t *p = surface.back;
    int total = w * h;

    /* darktable-inspired perceptual math */
    float gamma = adj_brightness >= 0
        ? 1.0f / (1.0f + adj_brightness / 100.0f)
        : 1.0f - adj_brightness / 100.0f;
    float contrast = (adj_contrast + 100) / 100.0f; /* 0..2 */
    float middle_grey = 0.1842f;
    float inv_mg = 1.0f / middle_grey;
    float sat_factor = (adj_saturation + 100) / 100.0f;

    if (adj_brightness || adj_contrast || adj_saturation || adj_hue) {
        for (int i = 0; i < total; i++) {
            uint32_t c = p[i];
            float r = RB_R(c) / 255.0f, g = RB_G(c) / 255.0f, b = RB_B(c) / 255.0f;

            /* Brightness: gamma correction (darktable basicadj) */
            if (adj_brightness) {
                r = powf(r > 0 ? r : 0, gamma);
                g = powf(g > 0 ? g : 0, gamma);
                b = powf(b > 0 ? b : 0, gamma);
            }

            /* Contrast: power curve around middle grey (darktable basicadj) */
            if (adj_contrast) {
                r = powf(r * inv_mg, contrast) * middle_grey;
                g = powf(g * inv_mg, contrast) * middle_grey;
                b = powf(b * inv_mg, contrast) * middle_grey;
            }

            /* Saturation: perceptual around luminance (darktable basicadj) */
...
EMSCRIPTEN_KEEPALIVE
void vz_blend_buffer(float intensity) {
...
EMSCRIPTEN_KEEPALIVE
void vz_set_bg_mode(int mode) { bg_mode = mode; }

EMSCRIPTEN_KEEPALIVE
void vz_set_bg_blur(int radius) { bg_blur_radius = radius; }

EMSCRIPTEN_KEEPALIVE
int vz_get_bg_image_buffer(void) {
    int total = surface.width * surface.height;
    if (!bg_image_buffer) {
...
EMSCRIPTEN_KEEPALIVE
void vz_set_face_landmark(int idx, float x, float y) {
    if (idx >= 0 && idx < 478) {
...
EMSCRIPTEN_KEEPALIVE
void vz_set_face_landmark_count(int n) {
    face_landmark_count = n > 478 ? 478 : n;
}

EMSCRIPTEN_KEEPALIVE
void vz_set_pip_position(int x, int y) { pip_pos_x = x; pip_pos_y = y; }

EMSCRIPTEN_KEEPALIVE
void vz_set_pip_opacity(float op) { pip_opacity = op; }

EMSCRIPTEN_KEEPALIVE
void vz_set_blend_mode(int mode) { blend_mode = mode; }

EMSCRIPTEN_KEEPALIVE
void vz_set_hair_color(int r, int g, int b) { hair_color = RB_RGB(r, g, b); }

EMSCRIPTEN_KEEPALIVE
void vz_set_hair_intensity(float val) {
    hair_intensity = val < 0.1f ? 0.1f : (val > 1.0f ? 1.0f : val);
}

EMSCRIPTEN_KEEPALIVE
void vz_set_subtitle_decay(int frames) {
    if (frames > 0) subtitle_decay_frames = frames;
}

EMSCRIPTEN_KEEPALIVE
int vz_get_face_mask_buffer(void) {
    int total = surface.width * surface.height;
...
EMSCRIPTEN_KEEPALIVE
void vz_set_outline_color(int r, int g, int b) { outline_color = RB_RGB(r, g, b); }

EMSCRIPTEN_KEEPALIVE
void vz_set_outline_thickness(int t) { outline_thickness = t > 4 ? 4 : (t < 1 ? 1 : t); }

EMSCRIPTEN_KEEPALIVE
void vz_set_tiltshift(int focus, int range) { tiltshift_focus = focus; tiltshift_range = range; }

/* ─── Text overlay (draw timecode etc on the frame) ─── */

EMSCRIPTEN_KEEPALIVE
void vz_draw_text(int x, int y, const char *str, int r, int g, int b) {
    rb_text(&surface, x, y, str, RB_RGB(r, g, b));
}

EMSCRIPTEN_KEEPALIVE
void vz_draw_rect(int x, int y, int w, int h, int r, int g, int b) {
