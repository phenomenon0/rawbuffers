/*
 * Web piano demo — keyboard piano with FFT visualization
 * Tests: canvas rendering, keyboard input, Web Audio synthesis
 */
...
static void draw(rb_ctx_t *ctx) {
    ...
    int n_s = (int)(got / sizeof(float));
    for (int i = n_s; i < FFT_SIZE; i++) samples[i] = 0.0f;
    ...
    for (int i = 0; i < num_bars; i++) {
        ...
        rb_rect_fill(s, bx, by, bar_w, bh, rb_hue((float)i / num_bars));
    }
    ...
    for (int i = 0; i < NUM_KEYS; i++) {
        ...
        uint32_t color = pressed ? rb_hue((float)i / NUM_KEYS)
                                 : (i < 7 ? RB_RGB(240, 235, 220) : RB_RGB(200, 220, 240));
    ...
    rb_text(s, margin, margin, "PIANO + FFT (WASM)", RB_RGB(255, 255, 255));
    rb_text(s, margin, margin + (int)(12 * sc), "ZXCVBNM / ASDFGHJK", RB_RGB(140, 140, 160));

    char info[64];
    snprintf(info, sizeof(info), "FPS:%d", ctx->fps);
